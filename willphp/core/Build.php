...
class Build
{
    ...
    public function initApp(): void
    {
        ...
        if (!is_writable($lock)) {
            ...
            $tplPath = THEME_ON ? VIEW_PATH . '/' . Config::init()->get('site.theme', 'default') : VIEW_PATH;
            $dirs = [APP_PATH . '/config', APP_PATH . '/controller', APP_PATH . '/model', APP_PATH . '/widget', $tplPath . '/public', $tplPath . '/index'];
            foreach ($dirs as $dir) {
                if (!is_dir($dir)) {
                    mkdir($dir, 0755, true);
                }
            ...
            file_put_contents($tplPath . '/index/index.html', $t_index);
            //转跳模板
            $t_jump = file_get_contents(ROOT_PATH . '/willphp/tpl/jump.tpl');
            file_put_contents($tplPath . '/public/jump.html', $t_jump);
            unlink($lock);
        }
    }
}